...
export const SystemTransition:React.FC<Props>=({mode,onComplete})=>{
  ...
  return <div className={`system-transition ${splash?'system-transition-splash fixed inset-0 z-[100]':'fixed inset-0 z-[90] bg-black/65 backdrop-blur-sm'} ${exiting?'system-transition-exit':''} grid place-items-center`} role={splash?'status':'dialog'} aria-live="polite" aria-label={splash?'Loading Vansidian':'Connecting wallet'}>
